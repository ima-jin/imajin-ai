## 28. Launch Readiness — The Critical Path from Demo to Fundraise ✅ RESOLVED 2026-04-22 (with extraction)

**Resolution summary:** P28 was a time-boxed launch-readiness document for the April 1 demo. Its pre-demo wiring (§3.1–§3.3) shipped on time. Its post-demo items partially shipped (issue triage 119 → 108) or did not (founding supporter tier). Temporal window has closed; the document is retired.

**What shipped (April 22 verification):**
- **§3.1 `.fair` to event transactions** — `settleTicketPurchase()` live (P8 resolved). PR #638 wired `.fair` manifests across all upload paths (P9 resolved).
- **§3.2 Events webhook → settlement** — events webhook calls settle on Stripe confirmation.
- **§3.3 Platform fee recording** — superseded by fee model v3 (four-layer: 1% protocol + 0.5% node + 0.25% user credit + 0.25% scope = 2.0%). `PLATFORM_DID` / `NODE_OPERATOR_DID` questions resolved structurally.
- **§2 `event.attended` attestation** — `event.attendance` shipped as part of the 34 ATTESTATION_TYPES vocabulary.
- **§6 Week 0** (pre-demo wiring) — done.
- **§6 Week 1 issue triage** — partial (119 → 108 open; missed <70 target but directionally correct).
- **§6 Week 2 business plan draft** — approaching v1 finalization per business plan memory.

**What was extracted (forwarded to own proposal):**
- **§4 Founding Supporter Tier** → `current-proposals/36-founding-supporter-tier.md`. The attestation primitive (`supporter.{tier}`), the rate lock-in semantics, the MJN-vs-MJNx decision, the securities framing, and the coffee/tip wiring are still live gaps and still load-bearing for the post-demo → fundraise bridge. #474 and #433 both still OPEN on April 22.

**What retired with this document:**
- **§1 Situation table** — status framing obsolete post-demo.
- **§5 Issue triage as a tracked proposal** — partially done; remaining cleanup is operational, not a protocol proposal. Does not carry forward.
- **§6 Three-week sequence** — time-boxed; all three weeks are in the past.
- **§7 Risk assessment** — historical record of pre-launch risk thinking.
- **§8 Open questions for Ryan** — all resolved by shipped work or superseded by fee model v3 decisions.
- **§9 Detection checklist** — first 4 boxes checked; last 2 carried into P36.

**File:** retired from `current-proposals/`; preserved here with original substance intact below this header for historical reference.
**Detected:** March 27, 2026 (filed)
**Resolved:** April 22, 2026 (with §4 extraction to P36)
...
**Connects to:** P29 (Attestation Completeness), **P36 (Founding Supporter Tier — extracted from §4 on resolution)**
