...
## App-delegated attestations: `attest:<appId>:<type>` (#2394)

`payload.delegator_did` was purpose-built for the intro-funnel vocabulary (#1885/#1895/#1897): the only capability that could ever back a delegated attestation was the closed `intros:propose` scope. #2394 generalizes the same envelope field to any registered third-party app's own attestation type, so a genuine respondent-signed fact (e.g. dykil's "DID X said Y about survey `<docHash>`") can be honestly recorded as "the app signed this, but the delegator authorized it" — without the kernel ever custodying or signing under the user's own DID key.

**Capability naming.** `packages/auth/src/grant-scopes.ts`'s `buildAttestDelegationCapability(appId, type)` builds `attest:<appId>:<type>`, where `appId` is the app's `registry.apps.id` (#1990) and `type` is any attestation type name (a compile-time `ATTESTATION_TYPES` entry or a live `attestation_type_registry` entry, above). This sits alongside, not inside, the closed `GRANT_SCOPE_REGISTRY` — it is validated structurally by `parseAttestDelegationCapability` (DB-free) and then against the live app registry + attestation-type vocabulary by `apps/kernel/src/lib/auth/attest-delegation.ts` (DB-aware), which additionally requires the named app's own DID to equal the grant's `agentDid` — a delegator can never be tricked into granting one app's capability slot to a different app.

**Issuance.** The user consents in their own kernel session: `POST /auth/api/grants` already requires a directly authenticated delegator (#1882); #2394 only extended its capability validation to also accept this shape. There is currently no dedicated consent-review UI for a third-party app's specific `attest:<appId>:<type>` request (a `gap(kernel)` issue was filed for that, refs #2394) — today a delegator must already know the exact capability string to grant it.

**Verification.** `POST /api/attestations` resolves `issuer_did`'s Ed25519 public key from `auth.identities` first, then falls back to `registry.apps` (a registered app is never mirrored into `auth.identities` — see `POST /api/registry/apps`; this is the same source of truth `POST /auth/api/apps/token`'s proof-of-possession check uses). When the issuer resolves to a live app and the type has no platform funnel capability, `verifyDelegatedAttestation` (`attestation-helpers.ts`) builds `attest:<appId>:<type>` and checks it the same way as any other delegation capability — a live, unexpired, unrevoked grant from `delegator_did` to `issuer_did`. The stored attestation records both `issuerDid` (the app) and `delegatorDid` (the user), and `GET /api/attestations` returns `delegatorDid` on every row so consumers can distinguish a delegated attestation from a self-signed one.

**Caller auth.** Per Ryan's 2026-09-26 ruling, an app authenticates its own inbound `POST /api/attestations` call with a scoped app-token (`POST /auth/api/tokens/app`, minted from the delegator's own kernel session and scoped to the app's host) rather than its own kernel session — `resolveCallerDid` in `route.ts` accepts this alongside the pre-existing session-cookie and legacy-bearer paths, re-checking the token's `aud` against the live app registry on every call (#1990).

**SDK.** `@imajin/auth`'s `submitDelegatedAttestation()` signs and submits the request with the app's own Ed25519 key, matching the kernel's canonical form exactly; `@imajin/auth-client`'s `requestAppToken()` mints the scoped app-token a federated app presents back as `Authorization: Bearer`.
