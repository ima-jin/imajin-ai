...
describe('dykil middleware — standalone dashboard redirect (#2332)', () => {
  ...
  it('308-redirects the standalone /dashboard route (app/(chrome)/dashboard) to its hub tab, preserving the query string', () => {
    const res = middleware(new NextRequest('https://jin.imajin.ai/dashboard?foo=bar'));

    expect(res.status).toBe(308);
    expect(res.headers.get('location')).toBe('https://jin.imajin.ai/auth/dykil?foo=bar');
  });
