...
export default async function Home({ searchParams }: { searchParams: SearchParams }) {
  // Read and verify session directly from cookie (no self-HTTP call)
  ...
  if (!sessionDid) {
    return (
      <div className="max-w-2xl mx-auto text-center py-20">
        ...
        <h1 className="text-3xl font-bold text-white mb-3">Attestation Browser</h1>
        <p className="text-zinc-400 mb-8">
          Sign in to browse attestations issued by or about your identity.
        </p>
        <Link
          href="/login"
  ...
  const myIdentity = identityMap.get(sessionDid);
