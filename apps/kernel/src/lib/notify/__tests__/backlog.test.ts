/**
 * Tests for `getNotificationBacklog` (#2044) — the backlog replay
 * `ws-server.js` triggers immediately after a DID reconnects.
 *
 * What matters here: only undelivered + unread rows are candidates, oldest
 * first; each candidate is claimed one at a time so a row that a concurrent
 * live push already claimed is skipped rather than delivered twice; the
 * result reports truncation without needing a separate COUNT query.
 */
import { describe, it, expect, vi, beforeEach } from 'vitest';

const { mockLimit, mockClaim } = vi.hoisted(() => ({
