 ...
 * bound to — and that failure is silent: a crossed wire seals an xAI key into
 * another provider's vault field, or disconnects the wrong credential. Factory
 * behaviour itself is covered in `src/lib/kernel/__tests__/`.
 */
...
describe('token route', () => {
  it('seals through the xAI connector, and nothing else', () => {
    expect(tokenOpts.current).toMatchObject({ name: 'xAI' });
...
describe('disconnect route', () => {
  it('revokes the xAI grant', () => {
    expect(disconnectOpts.current).toMatchObject({ name: 'xAI' });
...
describe('scope-manifest route', () => {
  it('validates fail-closed against the derived xAI scope list', () => {
    expect(manifestOpts.current).toMatchObject({ name: 'xAI', validScopes: ['xai:infer'] });
