...
describe('google oauth-state (#2144)', () => {
  ...
  it('round-trips a returnTo path (#1529)', () => {
    const verified = verifyState(signState(OWNER, '/auth/connectors/google'));
    expect(verified.did).toBe(OWNER);
    expect(verified.returnTo).toBe('/auth/connectors/google');
  });

  it('rejects a state whose returnTo was swapped for an off-origin URL (#1529)', () => {
    const [payloadB64, sig] = signState(OWNER, '/auth/connectors/google').split('.');
