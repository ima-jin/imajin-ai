 ...
 *
 * inference_capture — trigger the inference pipeline from an asset or text.
 * inference_status  — read session status + attestation for the agent.
 *
 * Both tools require the 'inference:write' scope gate (agents that can write
 * media can also trigger inference — add a separate scope if needed later).
 */
