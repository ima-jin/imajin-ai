 ...
 *
 * Two things the publish alone does not cover, and this factory does:
 *
 ...
 *   2. **Honest failure.** The requirement is fail-closed: a revoke that only
 ...
 *   4. Re-read the active scopes. Anything left means the revoke did not take,
 *      and the response says so with a 500 rather than claiming success.
 *   5. Publish a `connector.disconnected` bus event for the audit trail
...
export function createNativeDisconnectHandler(opts: NativeDisconnectOpts) {
  ...
  async function POST(request: NextRequest): Promise<NextResponse> {
    ...
    // 3. Verify. Reporting `connected: false` while a grant is still live would
    ...
    // 4. Audit trail. Same topic and payload shape as the OAuth/token-paste
