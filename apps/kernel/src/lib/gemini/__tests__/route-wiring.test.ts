...
const geminiManifestRoute = await import('../../../../app/gemini/api/scope-manifest/route');
...
it('re-exports POST and OPTIONS from the scope-manifest route, not just GET', () => {
  expect(expectScopeManifestRouteExportsPostAndOptions(geminiManifestRoute as Record<string, unknown>)).toBe(true);
});
...
describeRouteWiringContract({
  ...
  manifestRoute: geminiManifestRoute as Record<string, unknown>,
  sealApiKey,
  keySealed,
  keyPending,
  revokeApiKey,
