...
const zaiManifestRoute = await import('../../../../app/zai/api/scope-manifest/route');
...
it('re-exports POST and OPTIONS from the scope-manifest route, not just GET', () => {
  expect(expectScopeManifestRouteExportsPostAndOptions(zaiManifestRoute as Record<string, unknown>)).toBe(true);
});
...
describeRouteWiringContract({
  ...
  manifestRoute: zaiManifestRoute as Record<string, unknown>,
  sealApiKey,
  keySealed,
  keyPending,
  revokeApiKey,
