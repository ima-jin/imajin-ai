  ...
  return (
    <div className="max-w-lg mx-auto py-8">
      <div className="bg-[#0a0a0a] border border-white/10 p-8">
        <div className="mb-6">
          <a href="/auth/groups" className="text-sm text-muted hover:text-zinc-300 transition">
            ← Back to identities
          </a>
          <h1 className="text-2xl font-bold text-primary mt-3 mb-1 font-mono">Create Identity</h1>
          <p className="text-secondary text-sm">Set up a new group, organization, or family identity.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {fixedScope ? (
            <div className="flex items-center gap-2 pb-1">
              ...
              <span className="text-sm font-semibold text-warning uppercase tracking-wider">{scopeMeta.label}</span>
            </div>
          ) : (
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {SCOPES.map(s => (
                  ...
                  >
                    ...
                    <p className={`text-sm font-medium mt-1 ${scope === s.value ? 'text-warning' : 'text-zinc-300'}`}>
                      {s.label}
                    </p>
          ...
          <div>
            <label className="block text-xs font-semibold text-secondary uppercase tracking-wider mb-1.5">
              Bio / Description <span className="text-muted">(optional)</span>
            </label>
            <textarea
              value={description}
          ...
          {subtypeOptions.length > 0 && (
            <div>
              <label className="block text-xs font-semibold text-secondary uppercase tracking-wider mb-1.5">
                Subtype <span className="text-muted">(optional)</span>
              </label>
          ...
          <div className="flex gap-3 pt-2">
            <a
              href="/auth/groups"
              className="flex-1 px-4 py-2.5 bg-surface-base border border-white/10 text-secondary hover:text-primary hover:border-gray-500 transition-colors text-sm font-medium text-center"
            >
              Cancel
            </a>
            <button
