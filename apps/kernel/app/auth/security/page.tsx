  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-center text-secondary">Loading security settings…</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] p-4 md:p-8">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary mb-1 font-mono">Security settings</h1>
          <p className="text-secondary text-sm">Manage how you authenticate and protect your account.</p>
        </div>

        {/* Status message */}
        {statusMessage && (
          <div className={`p-4 border ${statusMessage.type === 'success' ? 'bg-success/20 border-green-800 text-success' : 'bg-error/20 border-red-800 text-error'}`}>
            {statusMessage.text}
          </div>
        )}
        ...
        <div className="bg-[#0a0a0a] border border-white/10 p-8">
          <h2 className="text-lg font-semibold text-primary mb-6 font-mono">Authentication methods</h2>

          {/* Key */}
          <div className="flex items-start justify-between py-4 border-b border-white/10">
            <div>
              <p className="text-primary font-medium">Cryptographic key</p>
              <p className="text-sm text-secondary mt-1">Always on — your sovereign identity. Your key is the root of all auth.</p>
            </div>
            <span className="px-2 py-1 text-xs bg-success/30 border border-green-800 text-success whitespace-nowrap ml-4">Always active</span>
          </div>
          ...
          <div className="py-4">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-primary font-medium">Password login</p>
              ...
              <div className="ml-4 flex flex-col items-end gap-2">
                {methods?.hasStoredKey ? (
                  <>
                    <span className="px-2 py-1 text-xs bg-success/30 border border-green-800 text-success whitespace-nowrap">Active</span>
                    <button
                      onClick={() => { setShowPasswordChange(true); setShowPasswordReset(false); setCurrentPassword(''); setPassword(''); setConfirmPassword(''); }}
                      className="text-sm px-3 py-1 bg-[#F59E0B] text-black hover:bg-[#D97706] transition"
                    >
                      Change password
                    </button>
                  </>
                  ...
                  <>
                    <span className="px-2 py-1 text-xs bg-surface-elevated border border-white/10 text-secondary whitespace-nowrap">Not set up</span>
                    <button
            ...
            {showPasswordChange && methods?.hasStoredKey && (
              <div className="mt-4 p-4 bg-surface-surface border border-white/10">
                <h3 className="text-primary font-medium mb-2 font-mono">Change password</h3>
                ...
                <form onSubmit={handlePasswordChange} className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-2 bg-[#F59E0B] text-black hover:bg-[#D97706] transition font-medium disabled:opacity-50"
                    >
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-sm text-warning hover:text-warning transition"
                  >
                    Forgot password?
                  </button>
                </div>
            ...
            {showPasswordReset && methods?.hasStoredKey && (
              <div className="mt-4 p-4 bg-warning/10 border border-warning/50">
                <h3 className="text-primary font-medium mb-2 font-mono">Reset password from this device</h3>
                ...
                <form onSubmit={handlePasswordReset} className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 py-2 bg-warning text-black hover:bg-warning transition font-medium disabled:opacity-50"
                    >
                ...
                <button
                  ...
                  className="mt-3 text-sm text-secondary hover:text-primary transition"
                >
                  Cancel
                </button>
              </div>
            ...
            {showPasswordSetup && !methods?.hasStoredKey && (
              <div className="mt-4 p-4 bg-surface-surface border border-white/10">
                <h3 className="text-primary font-medium mb-2 font-mono">Set up password login</h3>
                <p className="text-sm text-secondary mb-4">
        ...
        {/* MFA methods */}
        <div className="bg-[#0a0a0a] border border-white/10 p-8">
          <h2 className="text-lg font-semibold text-primary mb-2 font-mono">Multi-factor authentication</h2>
          <p className="text-sm text-secondary mb-6">Add a second factor to protect logins after key authentication.</p>

          {/* TOTP */}
          <div className="py-4 border-b border-white/10">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-primary font-medium">Authenticator app (TOTP)</p>
                <p className="text-sm text-secondary mt-1">Use an app like Authy or Google Authenticator to generate codes.</p>
              </div>
              <div className="ml-4 flex flex-col items-end gap-2">
                {hasTotpEnabled ? (
                  <span className="px-2 py-1 text-xs bg-success/30 border border-green-800 text-success whitespace-nowrap">Active</span>
                ...
                {hasTotpEnabled ? (
                  <button
                    ...
                    className="text-sm px-3 py-1 border border-red-800 text-error hover:bg-error/20 transition"
                  >
                    Remove
                  </button>
                ) : (
                  <button
                    ...
                    className="text-sm px-3 py-1 bg-[#F59E0B] text-black hover:bg-[#D97706] transition disabled:opacity-50"
                  >
            ...
            {showTotpSetup && totpSetup && (
              <div className="mt-4 p-4 bg-surface-surface border border-white/10">
                <h3 className="text-primary font-medium mb-3 font-mono">Scan QR code</h3>
                <p className="text-sm text-secondary mb-4">Scan this code with your authenticator app, then enter the 6-digit code to confirm.</p>
                <div className="flex justify-center mb-4">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={totpSetup.qrCode} alt="TOTP QR Code" className="" width={200} height={200} />
                </div>
                <p className="text-xs text-secondary text-center mb-4 font-mono break-all">
                ...
                <form onSubmit={handleVerifyTotp} className="flex gap-2">
                  <input
                    ...
                    maxLength={6}
                    autoFocus
                    className="flex-1 px-4 py-2 border border-white/10 bg-surface-base text-primary text-center font-mono tracking-widest focus:ring-2 focus:ring-[#F59E0B] focus:border-transparent"
                  />
                  <button
                    type="submit"
                    disabled={totpCode.length !== 6 || actionLoading === 'totp-verify'}
                    className="px-4 py-2 bg-[#F59E0B] text-black hover:bg-[#D97706] transition font-medium disabled:opacity-50"
                  >
            ...
            {/* TOTP disable flow */}
            {showTotpDisable && (
              <div className="mt-4 p-4 bg-error/10 border border-red-800/50">
                <h3 className="text-primary font-medium mb-2 font-mono">Confirm removal</h3>
                <p className="text-sm text-secondary mb-4">Enter your current authenticator code to remove TOTP.</p>
                <form onSubmit={handleDisableTotp} className="flex gap-2">
                  <input
                    ...
                    maxLength={6}
                    autoFocus
                    className="flex-1 px-4 py-2 border border-red-800 bg-surface-base text-primary text-center font-mono tracking-widest focus:ring-2 focus:ring-red-600 focus:border-transparent"
                  />
                  <button
                    type="submit"
                    disabled={totpDisableCode.length !== 6 || actionLoading === 'totp-disable'}
                    className="px-4 py-2 border border-red-700 text-error hover:bg-error/30 transition disabled:opacity-50"
                  >
          ...
          <div className="py-4 border-b border-white/10">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-primary font-medium">Email code</p>
                <p className="text-sm text-secondary mt-1">Receive a one-time code via email as a second factor.</p>
              </div>
              <div className="ml-4 flex flex-col items-end gap-2">
                {hasEmailMfa ? (
                  <span className="px-2 py-1 text-xs bg-success/30 border border-green-800 text-success whitespace-nowrap">Active</span>
                ...
                {hasEmailMfa ? (
                  <button
                    ...
                    className="text-sm px-3 py-1 border border-red-800 text-error hover:bg-error/20 transition disabled:opacity-50"
                  >
            ...
            {showEmailSetup && !hasEmailMfa && (
              <div className="mt-4 p-4 bg-surface-surface border border-white/10">
                <h3 className="text-primary font-medium mb-2 font-mono">Verify your email</h3>
                <p className="text-sm text-secondary mb-4">A 6-digit code was sent to your registered email address. Enter it below to activate email MFA.</p>
          ...
          <div className="py-4 opacity-50">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-primary font-medium">SMS</p>
                <p className="text-sm text-secondary mt-1">Receive a code via text message.</p>
              </div>
              <span className="ml-4 px-2 py-1 text-xs bg-surface-elevated border border-white/10 text-secondary whitespace-nowrap">Coming soon</span>
            </div>
          </div>
        </div>
        ...
        {/* Session duration */}
        <div className="bg-[#0a0a0a] border border-white/10 p-8">
          <h2 className="text-lg font-semibold text-primary mb-2 font-mono">Session duration</h2>
          <p className="text-sm text-secondary mb-4">How long you stay logged in on this device after signing in.</p>
          <select defaultValue="7d" className="w-full px-4 py-2 border border-white/10 bg-surface-base text-primary focus:ring-2 focus:ring-[#F59E0B] focus:border-transparent">
            <option value="1d">1 day</option>
            <option value="7d">7 days</option>
            <option value="28d">28 days</option>
            <option value="180d">6 months</option>
          </select>
          <p className="text-xs text-muted mt-2">Applied when session cookie is set on this device.</p>
        </div>

        {/* Trusted devices */}
        <div className="bg-[#0a0a0a] border border-white/10 p-8">
          <h2 className="text-lg font-semibold text-primary mb-2 font-mono">Known devices</h2>
          <p className="text-sm text-secondary mb-6">Devices that have been used to access your account.</p>

          {devices.length === 0 ? (
            <p className="text-sm text-secondary">No devices recorded yet.</p>
          ) : (
            <div className="space-y-3">
              {devices.map(device => (
                <div key={device.id} className="flex items-start justify-between p-3 bg-surface-surface border border-white/10">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm text-primary truncate">{truncateUserAgent(device.userAgent)}</p>
                      {device.trusted && (
                        <span className="px-1.5 py-0.5 text-xs bg-success/30 border border-green-800 text-success whitespace-nowrap">Trusted</span>
                      )}
                    </div>
                  ...
                  <div className="flex gap-2 ml-3 flex-shrink-0">
                    {!device.trusted && (
                      <button
                        ...
                        className="text-xs px-2 py-1 border border-white/10 text-secondary hover:bg-surface-elevated transition disabled:opacity-50"
                      >
                        Trust
                      </button>
                    )}
                    <button
                      ...
                      className="text-xs px-2 py-1 border border-red-800 text-error hover:bg-error/20 transition disabled:opacity-50"
                    >
                      Remove
                    </button>
                  </div>
