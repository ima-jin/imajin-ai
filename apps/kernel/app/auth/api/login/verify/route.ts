  ...
  try {
    ...
    if (body.dfosChain) {
      try {
        ...
        log.error({ err: String(err), identityId: identity.id }, '[login] DFOS bridge failed');
      }
    }
    ...
    import('@/src/lib/auth/dfos').then(async ({ getChainByImajinDid, checkRelayChain, ingestToRelay }) => {
      ...
      if (!onRelay) {
        ...
        if (ok) {
          log.info({ identityId: identity.id, dfosDid: chain.dfosDid }, '[login] Backfilled relay chain');
        }
      }
    }).catch(err => log.error({ err: String(err), identityId: identity.id }, '[login] Relay backfill failed'));
    ...
    publish('session.created', {
      ...
      subject: identity.id,
      scope: 'kernel',
      payload: { tier: identity.tier || 'preliminary' },
    });
    ...
    }).catch(err => log.error({ err: String(err) }, '[login] Device log failed'));
