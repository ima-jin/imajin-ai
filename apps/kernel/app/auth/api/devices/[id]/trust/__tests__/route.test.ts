...
describe('POST /auth/api/devices/[id]/trust (#306)', () => {
  ...
  it('returns 404 for an unknown device id', async () => {
    ...
    const res = await POST(makeReq(), makeProps('dev_missing'));
  ...
  it('returns 403 when the device belongs to a different DID', async () => {
    ...
    h.foundDevice = { id: 'dev_1', did: OWNER_DID, trusted: false };
  ...
  it('marks the device trusted for its own DID', async () => {
    h.requireAuth.mockResolvedValue({ sub: OWNER_DID });
    h.foundDevice = { id: 'dev_1', did: OWNER_DID, trusted: false };
    h.updatedDevice = { id: 'dev_1', did: OWNER_DID, trusted: true };
