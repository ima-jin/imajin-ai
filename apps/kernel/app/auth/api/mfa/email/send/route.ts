...
import { verifyMfaChallengeToken } from '@/src/lib/auth/jwt';
...
export const POST = withLogger('kernel', async (request: NextRequest, { log }) => {
  ...
  try {
    ...
    try {
      ...
      log.error({ err: String(err) }, '[mfa/email/send] Email send failed (non-fatal)');
    }

    return NextResponse.json({ sent: true }, { headers: cors });

  } catch (error) {
    log.error({ err: String(error) }, '[mfa/email/send] POST error');
