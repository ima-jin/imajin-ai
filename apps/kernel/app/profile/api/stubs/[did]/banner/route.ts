...
const UPLOAD_DIR = '/mnt/media/banners';
...
/**
 * POST /profile/api/stubs/:did/banner
 * Upload a banner image for a stub business identity.
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    // Clean up old banners for this DID
    try {
    ...
    const url = `/api/media/banners/${filename}?v=${timestamp}`;
    ...
    log.info({ did, url }, '[stubs/banner] Banner updated');
    return NextResponse.json({ url });
  } catch (error) {
    log.error({ err: String(error) }, '[stubs/banner] Upload failed');
