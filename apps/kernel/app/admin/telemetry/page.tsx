...
export default async function TelemetryPage() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Telemetry</h1>
        ...
        {hours.map((h, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
              ...
              {i % 4 === 0 && (
                <span className="text-[9px] text-gray-400 leading-none">{h.label}</span>
              )}
            </div>
          );
        })}
          ...
          {rows.map((r, i) => (
            <tr key={i}>
              <td className="py-2 pr-4">
                ...
                <span className="font-mono text-xs text-gray-900 dark:text-gray-100">{r.path}</span>
              </td>
              <td className="py-2 text-right text-gray-600 dark:text-gray-400 tabular-nums">
                {r.total_requests.toLocaleString()}
              </td>
              <td className="py-2 text-right font-medium text-amber-600 dark:text-amber-400 tabular-nums">
                {r.p95}ms
              </td>
        ...
        <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
          {rows.map((r, i) => (
            <tr key={i}>
              ...
              <td className="py-2 text-right text-gray-600 dark:text-gray-400 tabular-nums">{r.p50}ms</td>
              <td className="py-2 text-right text-amber-600 dark:text-amber-400 tabular-nums">{r.p95}ms</td>
              <td className="py-2 text-right text-red-600 dark:text-red-400 tabular-nums">{r.p99}ms</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
          {rows.map((r) => (
            <tr key={r.id}>
              <td className="py-2 pr-3 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap tabular-nums">
                {new Date(r.created_at).toLocaleTimeString()}
              </td>
              <td className="py-2 pr-3 text-xs text-gray-700 dark:text-gray-300">{r.service}</td>
              <td className="py-2 pr-3 font-mono text-xs text-gray-900 dark:text-gray-100">
                <span className="text-gray-500 dark:text-gray-400 mr-1">{r.method}</span>
                {r.path}
              </td>
