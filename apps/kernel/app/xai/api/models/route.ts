 ...
 * with the Gemini connector's model picker (#1818). xAI's own API shape —
 * `listModels`/`probeModel` against xAI's OpenAI-compatible `/v1/models` —
 * comes from `createOpenAiCompatibleModelHandlers` (#1927), shared with the
 * OpenAI connector's model picker, which speaks the identical shape.
 *
...
const { listModels, probeModel } = createOpenAiCompatibleModelHandlers<XaiCredentials>(XAI_BASE_URL);
