/**
 * POST + OPTIONS /typesafe/api/disconnect (#2197, #1720 pattern)
 *
 * Revoke the sealed TypeSafe API key's delegation grant for the session
 ...
 * `kernel.vault_delegation_grants` row for `typesafe-api-key:{ownerDid}` is
 * marked revoked and its wrapped key material erased, so nothing can unseal
 * it until a fresh grant exists. Every active `typesafe` `channel_links` row
...
import { revokeApiKey } from '@/src/lib/typesafe/connector';
