-- 0118_connectors_registry_zai.sql
-- Extend the kernel.connectors shadow registry (0114) to the Z.ai (Zhipu AI,
-- GLM-4.x family) brain connector — Phase 1 of the inference connectors epic
-- (#1922), sub-issue #1931.
--
...
-- OpenAI (#1927); 0117 extended it to Moonshot (#1930). Z.ai did not exist as
-- a `CONNECTOR_REGISTRY` provider yet, so no owner could have an active
-- `auth.channel_links` row or a `zai-api-key:*` vault delegation grant —
-- there is nothing to backfill retroactively. This migration exists so the
-- registry's backfill logic is generalized to Z.ai going forward (a fresh
-- `zai-api-key:*` grant sealed moments before this migration runs, in the
...
--
-- Same shape as 0114/0115/0117: full-outer-join the two independent sources,
-- and INSERT ... ON CONFLICT DO NOTHING so a re-run is a no-op and cannot
-- clobber a row the application already wrote.

WITH provider_map (provider, channel, connector_did, sealed_key_prefix) AS (
  VALUES
    ('zai', 'zai', 'did:imajin:zai-connector', 'zai-api-key')
),
linked AS (
