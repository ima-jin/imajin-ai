...
-- provider known at that time; 0115/0117/0118 extended it to OpenAI,
-- Moonshot, and Z.ai respectively. OpenRouter did not exist as a
-- `CONNECTOR_REGISTRY` provider yet, so no owner could have an active
-- `auth.channel_links` row or an `openrouter-api-key:*` vault delegation
-- grant — there is nothing to backfill retroactively. This migration exists
-- so the registry's backfill logic is generalized to OpenRouter going
-- forward (a fresh `openrouter-api-key:*` grant sealed moments before this
-- migration runs, in the gap between the application deploy and this
-- migration executing, would otherwise be invisible to the registry until
-- its next seal/revoke/publish).
--
-- Same shape as 0114/0115/0117/0118: full-outer-join the two independent
-- sources, and INSERT ... ON CONFLICT DO NOTHING so a re-run is a no-op and
-- cannot clobber a row the application already wrote.

WITH provider_map (provider, channel, connector_did, sealed_key_prefix) AS (
  VALUES
    ('openrouter', 'openrouter', 'did:imajin:openrouter-connector', 'openrouter-api-key')
),
linked AS (
