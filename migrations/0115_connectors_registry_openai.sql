-- 0115_connectors_registry_openai.sql
-- Extend the kernel.connectors shadow registry (0114) to the OpenAI brain
-- connector — Phase 1 of the inference connectors epic (#1922), sub-issue
-- #1927.
--
-- 0114 already created `kernel.connectors` and backfilled it for every
-- provider known at that time, including xAI (#1924). OpenAI did not exist as
-- a `CONNECTOR_REGISTRY` provider yet, so no owner could have an active
-- `auth.channel_links` row or a `openai-api-key:*` vault delegation grant —
-- there is nothing to backfill retroactively. This migration exists so the
-- registry's backfill logic is generalized to OpenAI going forward (a fresh
-- `openai-api-key:*` grant sealed moments before this migration runs, in the
-- gap between the application deploy and this migration executing, would
-- otherwise be invisible to the registry until its next seal/revoke/publish).
--
-- Same shape as 0114: full-outer-join the two independent sources, and INSERT
-- ... ON CONFLICT DO NOTHING so a re-run is a no-op and cannot clobber a row
-- the application already wrote.

WITH provider_map (provider, channel, connector_did, sealed_key_prefix) AS (
  VALUES
    ('openai', 'openai', 'did:imajin:openai-connector', 'openai-api-key')
),
linked AS (
